...
struct arguments
{
	...
	std::string  fixedGroupList;	/* -F option */
	std::string  movingGroupList; /* -M option */
	...
	friend std::ostream& operator<< (std::ostream& o, const arguments& args)
	{
		...
		return o
			...
			<<"  List of images in the fixed group contained in: "<<args.fixedGroupList<<std::endl
			<<"  List of images in the moving group contained in: "<< args.movingGroupList<<std::endl
...
void parseOpts (int argc, char **argv, struct arguments & args)
{
	...
	command.SetOption("FixedHistogramField","F",true,"Filename containing list of images for building fixed histogram field");
	command.SetOptionLongTag("FixedHistogramField","fixed-histogram");
	command.AddOptionField("FixedHistogramField","filename",MetaCommand::STRING,true);

	command.SetOption("MovingHistogramField","M",true,"Filename containing list of images for building moving histogram field");
	command.SetOptionLongTag("MovingHistogramField","moving-histogram");
	command.AddOptionField("MovingHistogramField","filename",MetaCommand::STRING,true);


	//command.SetOption("LabelImageFile","L",true,"Label image filename");
	//command.SetOptionLongTag("LabelImageFile","Label-image");
	//command.AddOptionField("LabelImageFile","filename",MetaCommand::STRING,true);

	//command.SetOption("SecondImageFile","S",true,"Second image filename");
	//command.SetOptionLongTag("SecondImageFile","Second-image");
	//command.AddOptionField("SecondImageFile","filename",MetaCommand::STRING,true);
	...
	args.fixedGroupList = command.GetValueAsString("FixedHistogramField","filename");
	args.movingGroupList = command.GetValueAsString("MovingHistogramField","filename");
	//args.LabelImageFile = command.GetValueAsString("LabelImageFile", "filename");
	//args.SecondImageFile = command.GetValueAsString("SecondImageFile", "filename");
...
public:
	...
	typedef itk::LabelDiffeomorphicDemonsRegistrationFilter<
		InternalImageType,
		InternalImageType,
	...
	typedef itk::LabelMultiResolutionPDEDeformableRegistration<
...
void DemonsRegistrationFunction( arguments args )
{
	...
	typename HistogramFieldType::Pointer fixedHistogramField=new HistogramFieldType();
	typename HistogramFieldType::Pointer movingHistogramField=new HistogramFieldType();
	...
	typedef itk::ImageFileReader< ImageType >            SecondImageReaderType;
	...
	{//for mem allocations
		...
		typename SecondImageReaderType::Pointer secondImageReader
			= SecondImageReaderType::New();
		...
		try
		{
			...
			//secondImageReader->Update();
			std::cout<<"start loading the fixed group...";
			fixedHistogramField->LoadFromList(args.fixedGroupList.c_str());
			std::cout<<"done"<<std::endl;
			std::cout<<"start loading the moving group...";
			movingHistogramField->LoadFromList(args.movingGroupList.c_str());
			std::cout<<"done"<<std::endl;
		}
		...
		{
			assert(0);//not to be used so far
			// match intensities
			...
			typedef itk::HistogramMatchingImageFilter
				<MoveGradImageType, MoveGradImageType> GradMatchingFilterType;
			typename GradMatchingFilterType::Pointer GradMatcher= GradMatchingFilterType::New();
			...
			GradMatcher->SetInput( secondImageReader->GetOutput() );
			GradMatcher->SetReferenceImage( fixedImageReader->GetOutput() );

			GradMatcher->SetNumberOfHistogramLevels( 1024 );
			GradMatcher->SetNumberOfMatchPoints( 7 );
			GradMatcher->ThresholdAtMeanIntensityOn();
			...
			//movingGradImage= GradMatcher->GetOutput();
			//movingGradImage->DisconnectPipeline();

			//	fixedGradImage=fixedImage;
		}//if (!args.useHistogramMatching)
	...
	{//for mem allocations
		...
		typedef typename itk::LabelDiffeomorphicDemonsRegistrationFilter
		...
		case 0:
			{
				...
				actualfilter->SetUseGradientType(
					static_cast<itk::GradientType>(args.gradientType) );
				filter = actualfilter;
		...
		typedef typename itk::LabelMultiResolutionPDEDeformableRegistration<
		...
		multires->SetFixedHistogramField(fixedHistogramField);
		multires->SetMovingHistogramField(movingHistogramField);
